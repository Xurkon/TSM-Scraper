...
class TSMScraperApp(ctk.CTk if HAS_CTK else object):
    ...
    def delete_group_dialog(self, group_path: str):
        ...
        try:
            ...
            if result.get('ui_refs_removed', 0) > 0:
                msg += f" ({result['ui_refs_removed']} UI refs)"
    ...
    def update_manual_status(self):
        """Update the manual IDs status label."""
            ...
            for cat_name in categories:
                ...
                if is_wowhead:
                    ...
                    class_map = {
                        ...
                        "vanity": self.scraper.CLASS_VANITY,
                        "misc": self.scraper.CLASS_MISC,
                        "quiver": getattr(self.scraper, 'CLASS_QUIVER', 11),
                        "glyph": getattr(self.scraper, 'CLASS_GLYPH', 16),
                        "quest": getattr(self.scraper, 'CLASS_QUEST', 12),
                        "key": getattr(self.scraper, 'CLASS_KEY', 13),
                        "currency": getattr(self.scraper, 'CLASS_CURRENCY', 10),
                        "pet_whistle": getattr(self.scraper, 'CLASS_PET_WHISTLE', 101),
                    }
                    class_id = class_map.get(cat_type)
                    if class_id is None:
                        continue

                    # Build URL - handle slot filtering correctly
                    # For armor slots like "1.1" (cloth helm), the format is:
                    # ?items=4.1&filter=sl=1 (NOT ?items=4.1.1)
                    slot_filter = None
                    actual_subclass = subclass

                    if subclass is not None and isinstance(subclass, str) and '.' in str(subclass):
                        # Parse "subclass.slot" format (e.g., "1.1" -> subclass=1, slot=1)
                        parts = str(subclass).split('.')
                        actual_subclass = parts[0]
                        if len(parts) > 1:
                            slot_filter = parts[1]

                    url = f"{self.scraper.BASE_URL}/?items={class_id}"
                    if actual_subclass is not None:
                        url += f".{actual_subclass}"

                    # Build filter parameters
                    # Ascension DB filter format: &filter=cr=X;crs=1;crv=0;sl=Y
                    filter_parts = []

                    # Add bind filter for Ascension DB
                    # GUI bind IDs: 1=BoP, 2=BoE, 3=BoU, 4=Quest, 5=BoA
                    # Ascension DB filter cr values: 2=BoP, 3=BoE, 4=BoU
                    filter_selection = self.bind_filter_var.get()
                    bind_id, _ = self.BIND_FILTER_OPTIONS.get(filter_selection, (None, ""))
                    if bind_id is not None:
                        ...
                        cr_value = ASCENSION_BIND_MAP.get(bind_id)
                        if cr_value:
                            filter_parts.append(f"cr={cr_value}")
                            filter_parts.append("crs=1")
                            filter_parts.append("crv=0")

                    # Add slot filter if specified
                    if slot_filter:
                        filter_parts.append(f"sl={slot_filter}")

                    # Append filter to URL if we have any parts
                    if filter_parts:
    ...
    def run_import(self):
        ...
        try:
            ...
            # Pre-calculate and create all necessary groups first
            groups_to_create = set()
            
            # Manual items group
            if self.manual_add_ids and selected_group != "(Use default from scraper)":
                groups_to_create.add(selected_group)
                
            # Category groups
            selected_results = self.get_selected_import_categories()
            for cat_name, data in selected_results.items():
                if data['new_ids']:
                    target = selected_group if use_custom_group else data['tsm_group']
                    groups_to_create.add(target)
            
            if groups_to_create:
                self.log(f"Ensuring {len(groups_to_create)} groups exist...", 'info')
                writer.add_groups(list(groups_to_create), dry_run=False)
            ...
            # selected_results already retrieved above
    ...
    def create_default_groups(self):
        ...
        if not themed_askquestion(
            ...
            "Creating default groups will ADD 160 new groups to your TSM profile.\n"
